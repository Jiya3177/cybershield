...
import { getState, performAction, resetEnvironment, updateSettings } from "../api/api";
import {
  buildEventFeed,
  formatLastUpdated,
  getAlerts,
...
function Dashboard() {
  ...
  const compactMode = Boolean(state.agent_profile?.display_preferences?.compact_mode);
  const highContrastCharts = Boolean(state.agent_profile?.display_preferences?.high_contrast_charts);
  const sectionGapClass = compactMode ? "mt-4" : "mt-6";
  ...
  return (
    <div className="min-h-screen bg-cyber-bg text-white">
      ...
      <main className={`relative mx-auto max-w-[1500px] px-4 ${compactMode ? "py-4" : "py-6"} sm:px-6 lg:px-8`}>
        <Header
        ...
        <AlertsOverlay
          ...
          onDismiss={(alertId) => setDismissedAlerts((current) => [...current, alertId])}
        />

        <MetricsGrid metrics={metrics} />

        <section className={`${sectionGapClass} grid gap-6 2xl:grid-cols-12`}>
          <div className="2xl:col-span-3">
            <HealthPanel systemHealth={state.system_health} history={healthHistory} threatLevel={threatLevel} />
          </div>
          <div className="2xl:col-span-5">
            <AttackTimelineChart data={attackTimelineData} highContrast={highContrastCharts} />
          </div>
        ...
        <section className={`${sectionGapClass} grid gap-6 xl:grid-cols-12`}>
        ...
        <section className={`${sectionGapClass} grid gap-6 xl:grid-cols-12`}>
          ...
          <div className="xl:col-span-5">
            <AttackChart data={attackChartData} highContrast={highContrastCharts} />
          </div>
        ...
        <section className={`${sectionGapClass} grid gap-6 xl:grid-cols-12`}>
          <div className="xl:col-span-5">
            <SettingsPanel
              ...
              latestReward={state.latest_reward}
